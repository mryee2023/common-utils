...
// PingAnVerify 数据验签
func (r *XRsa) PingAnVerify(data string, sign string) error {
	hexByte, err := hex.DecodeString(sign)
	if err != nil {
		return err
	}
	h := RSA_ALGORITHM_SIGN.New()
	...
	return rsa.VerifyPKCS1v15(r.publicKey, RSA_ALGORITHM_SIGN, hashed, hexByte)
}

// PingAnSign 数据加签
func (r *XRsa) PingAnSign(data string) (string, error) {
	h := RSA_ALGORITHM_SIGN.New()
	h.Write([]byte(data))
	hashed := h.Sum(nil)
...
// NewPingAnXRsa 生成公钥私钥rsa-key
func NewPingAnXRsa(publicKey []byte, privateKey []byte) (*XRsa, error) {
	block, _ := pem.Decode(publicKey)
	if block == nil {
		return nil, errors.New("public key error")
	}
